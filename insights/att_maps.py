...
class AttentionMap:
    ...
    def load_model(self):
        loaded_model = torch.load('/scratch4/levans/tth-network/plots/Archives/transformer3/04_04_24/model.pt')
    ...
    def calculate_entropy(self, attention_weights):
        ...
        entropy = -np.sum(attention_weights_np * np.log(attention_weights_np + 1e-9), axis=1)  # Add a small number to avoid log(0)
        return entropy

    def plot_attention_weights(self, attention_weights, feature_names, layer_index=None,):
        ...
        for head_index in range(num_heads):
            ...
            plt.title(f'Layer {layer_index} Attention Weights (Head {head_index})' if layer_index is not None else f'Attention Weights (Head {head_index})')
            plt.colorbar()
            plt.tight_layout()

            plot_filename = f'attention_weights_layer{layer_index}_head{head_index}.png' if layer_index is not None else f'attention_weights_head{head_index}.png'
            plot_path = f'/scratch4/levans/tth-network/plots/Archives/transformer3/04_04_24/{plot_filename}'
            plt.savefig(plot_path)
            plt.close()

    def plot_attention_entropy(self,attention_weights, feature_names, layer_index=None):
        ...
        plt.tight_layout()
        plt.savefig(f'/scratch4/levans/tth-network/plots/Archives/transformer3/04_04_24/average_attention.png')
        plt.close()
        ...
        plt.xlabel('Sequence Position')
        plt.ylabel('Entropy')
        plt.savefig(f'/scratch4/levans/tth-network/plots/Archives/transformer3/04_04_24/attention_entropy.png')
        plt.close()
