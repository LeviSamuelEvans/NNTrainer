...
class CosineRampUpDownLR(_LRScheduler):
    def __init__(self, optimizer, lr_init=1e-8, lr_max=1e-5, lr_final=1e-7, burn_in=10, ramp_up=10, plateau=20, ramp_down=100, last_epoch=-1):
    ...
    def get_lr(self):
        ...
        if epoch < self.ramp_up:
            ...
            return [(self.lr_max - self.lr_init) * fraction + self.lr_init for _ in self.base_lrs]
        ...
        if epoch < self.ramp_down:
            ...
            return [(self.lr_max - self.lr_final) * fraction + self.lr_final for _ in self.base_lrs]
